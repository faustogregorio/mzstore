...
export class Slide {
    ...
    detectClickOnArrow(event: any) {
        return event.target.classList.contains("carousel-arrow");
    }

    handleSlide(customSlideLength: any = undefined): any {
    ...
    getSlideLength(distanceAbs: any) {
    ...
    getPositionWithoutCorrection(value: number): number {
        const remainder = value % this.fullCellWidth;
    ...
    isLightDOMMode(counter: number): boolean {
        let flag = false;
        const remainderOfCells = this.images.length - this.overflowCellsLimit - this.visibleCellsCount;
    ...
    ifLeftDOMModeAtEnd(counter: number): boolean {
        let flag = false;
        const remainderOfCells = this.images.length - this.overflowCellsLimit - this.visibleCellsCount;
    ...
    ifLeftDOMModeToBeginning(counter: number): boolean {
        let flag = false;
